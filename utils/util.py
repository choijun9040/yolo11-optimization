...
def compute_iou(box1, box2, eps=1e-7):
    ...
    rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4.0  # center dist ** 2
...
class Assigner(torch.nn.Module):
    def __init__(self, nc=16, top_k=13, alpha=1.0, beta=6.0, eps=1E-9):
        super().__init__()
        self.top_k = top_k
        self.nc = nc
        self.alpha = alpha
...
class ComputeLoss:
    ...
    def box_decode(self, anchor_points, pred_box_offsets):
        lt, rb = pred_box_offsets.chunk(2, -1)
        x1y1 = anchor_points - lt
        x2y2 = anchor_points + rb
        return torch.cat(tensors=(x1y1, x2y2), dim=-1)

    def __call__(self, outputs, targets):
        x = torch.cat([i.view(outputs[0].shape[0], i.shape[1], -1) for i in outputs], dim=2)
        pred_bboxes_padded, pred_scores = x.split(split_size=(16, self.nc), dim=1)
        pred_bboxes_direct = pred_bboxes_padded[:, :4]

        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_bboxes_direct = pred_bboxes_direct.permute(0, 2, 1).contiguous()
        ...
        pred_bboxes = self.box_decode(anchor_points, pred_bboxes_direct)
...
def post_process_yolo(prediction_outputs, stride):
    """
    A post-processing function that calculates the final bounding boxes from the raw output of the YOLO model. This function is intended to be executed on the CPU.

    Args:
        prediction_outputs (list of torch.Tensor): A list of raw tensors output by the model
        stride (torch.Tensor): The stride value for each prediction layer of the model
    """

    # Translate the raw output and stride to generate the anchor and stride grids using the make_anchors function
    anchors, strides = (i.transpose(0, 1) for i in make_anchors(prediction_outputs, stride))
    
    # Reshape and concatenate the list of tensors in the shape of [B, C, H, W] into a single tensor of shape [B, C, H*W]
    # C is the number of channels (16 + num_classes)
    num_outputs_per_anchor = prediction_outputs[0].shape[1] # 16 + nc
    x = torch.cat([i.view(prediction_outputs[0].shape[0], num_outputs_per_anchor, -1) for i in prediction_outputs], dim=2)

    nc = num_outputs_per_anchor - 16

    # Seperate the box information and the class information
    box_padded, cls = x.split(split_size=(16, nc), dim=1)
    
    # Use only the four actual coordinate values from the box information
    box = box_padded[:, :4, :]

    # Decoding Logic
    a, b = box.chunk(2, 1)
    
    # Match the device and date type of the anchors and strides to that of the boxes
    anchors = anchors.to(box.device, box.dtype)
    strides = strides.to(box.device, box.dtype)

    a = anchors.unsqueeze(0) - a
    b = anchors.unsqueeze(0) + b
    box_decoded = torch.cat(tensors=((a + b) / 2.0, b - a), dim=1) # (center_x, center_y, width, height)

    # finial_output: Concatenate the (coordinates * strides) and the (class probabilities)
    final_output = torch.cat(tensors=(box_decoded * strides, cls.sigmoid()), dim=1)
